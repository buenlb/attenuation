...
subjectInds = find(ismember(subjects, subjectsShape));
tmp1 = [];
tmp2 = [];
for i = subjectInds'
    tmp1 = [tmp1; bioheatMajAL{i}(aboveThreshInds{i})];
    tmp2 = [tmp2; thermomMajAL{i}(aboveThreshInds{i})];
end
mdl = fitlm(tmp1, tmp2);
...
subjectInds = ismember(subjects, subjectsShape);
mdl = fitlm(cell2mat(bioheatMinAL(subjectInds)), cell2mat(thermomMinAL(subjectInds)));
x = (0:xlims(2)/10:xlims(2))';
y = predict(mdl, x);
plot(x, y, 'k--', 'linewidth', 2);
text(0.1*xlims(2), 0.80*xlims(2), sprintf('r^2 = %0.3f', mdl.Rsquared.Adjusted));
...
% xlim([0,6]);
% ylim([0,6]);
% ylabel('Clinical major to minor axis ratio');
