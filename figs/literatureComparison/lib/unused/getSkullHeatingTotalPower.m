% Script to calculate the total power from the SkullHeating files
...
% get total power from SkullHeating files
for subjectInd = 1:length(targetSubjects)
    ...
    for i = 1:numSonications
        ...
        % SkullHeating file
        [skullHeating, ~] = readSkullHeatingFile(sys);
        sonicationPowers{i,subjectInd} = skullHeating(:, 5);
